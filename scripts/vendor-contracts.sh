#!/usr/bin/env bash
#
# A sibling-checkout-path sync would silently no-op in CI, which checks out one
# repo at a time, so a sibling path never exists there. This clones the pinned
# tag for real and verifies the resolved commit SHA against contracts.lock before
# copying anything, so a moved or re-pushed tag can't silently swap content.
set -euo pipefail
cd "$(dirname "$0")/.."
