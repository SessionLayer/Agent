...
## The identity, and why persistence matters

`dataDir` holds the Agent's mTLS identity. Losing it is not a slower start next
time: the Control Plane refuses to enrol a node whose identity is still active,
so the pod exits 4 and stays down.

```text
identity refused by the Control Plane - not transient, do not auto-restart
into a loop ... phase="agent enrollment" exit_code=4
```

Recovering means deleting the node in the Control Plane and issuing a fresh
join credential, by hand, per node. With `persistence.enabled=false` that
happens on every pod restart — a node reboot, an image bump, a rollout — which
for a DaemonSet is routine, and it happens whichever `join.method` you use,
because the refusal is on the identity, not on the credential.

`persistence.enabled=true` keeps the identity on the node, where the systemd
deployment keeps it too. Create the directory first, owned by the container's
uid:

```bash
install -d -o 65532 -g 65532 -m 0700 /var/lib/sessionlayer-agent
```

Kubernetes creates a `hostPath` root-owned `0755` and `fsGroup` does not apply
to one, so the Agent cannot write a directory Kubernetes made for it. That step
is yours; the chart does not run a privileged init container to do it for you.
...
list parsed by the Agent binary itself.
...
Also proven for this chart specifically: the DaemonSet joined a live Control
Plane with a single-use token, applied Landlock (`FullyEnforced`) and its
seccomp allow-list, and registered a control channel on a live Gateway. With
`persistence.enabled=true` the pod was then deleted with a deliberately invalid
token in the join Secret and came back Ready with the same `agent_id` and
`node_id` — the restart that is fatal without it. No SSH session was spliced:
the kind node runs no sshd.
