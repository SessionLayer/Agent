//! Tamper matrix: correctly signed binaries verify; unsigned, tampered, wrong-identity,
//! wrong-workflow binaries are refused (fail closed). Fixtures build real Fulcio-shaped chains.
...
fn build(p: Params) -> Fixture {
    ...
    let rekor_sk = p256::ecdsa::SigningKey::generate();
...
/// p256 0.14 / ecdsa 0.17 moved the low-S check into `verify_prehashed`, gated on
/// `EcdsaCurve::NORMALIZE_S` — but `NistP256`'s impl is `const NORMALIZE_S: bool =
/// false`, same as ecdsa 0.16.9 (which had no low-S check at all). So the
/// malleable "flipped" sibling of a valid signature (`s' = n - s`, still a valid
/// signature over the same message under RFC 6979/ECDSA math) must still verify
/// on P-256, exactly as it did before the bump. None of our call sites
/// (cosign blob, DSSE, Rekor SET, SCT) treat signature bytes as a
/// uniqueness/idempotency key, so this is safe — but it must be pinned down so a
/// future RustCrypto release that quietly starts enforcing low-S for P-256 fails
/// this test instead of silently rejecting previously-valid production signatures.
#[test]
fn high_s_malleable_signature_still_verifies_p256_enforces_no_low_s() {
    use p256::ecdsa::signature::hazmat::{PrehashSigner, PrehashVerifier};
    use p256::elliptic_curve::scalar::IsHigh;

    let sk = p256::ecdsa::SigningKey::generate();
    let vk = *sk.verifying_key();
    let digest = Sha256::digest(b"p256-malleability-probe");

    // Sign the prehash, not the message: `Signer::sign` would hash again, so the
    // signature would cover SHA256(SHA256(m)) while verify_prehash checks SHA256(m).
    let sig: p256::ecdsa::Signature = sk
        .sign_prehash(&digest)
        .expect("signing a 32-byte prehash on P-256");
    vk.verify_prehash(&digest, &sig)
        .expect("a freshly produced signature must verify");

    let flipped_s = -sig.s();
    let flipped = p256::ecdsa::Signature::from_scalars(sig.r(), flipped_s)
        .expect("r together with the negated s is still a well-formed signature");
    assert_ne!(sig.s(), flipped.s());
    assert_ne!(
        bool::from(sig.s().is_high()),
        bool::from(flipped.s().is_high()),
        "exactly one of {{s, n-s}} is high — otherwise this test proves nothing"
    );

    vk.verify_prehash(&digest, &flipped).expect(
        "P-256 has NORMALIZE_S = false: the high-S sibling of a valid signature must \
         still verify. If this starts failing, a RustCrypto bump enabled low-S \
         enforcement for P-256 -- report that as a security-relevant behaviour \
         change, do not just update this test to match.",
    );
}
...
fn refuses_signed_downgrade() {
    ...
    assert_eq!(std::fs::read(&live).unwrap(), b"CURRENT-2.0.0");
...
fn refuses_untrusted_rekor_key() {
    ...
    f.trust.rekor_keys = vec![RekorKey {
        key: *p256::ecdsa::SigningKey::generate().verifying_key(),
        valid_for: OPEN_WINDOW,
    }];
...
/// turned off by shipping a ctlog-less anchor. Defense-in-depth.
#[test]
...
fn golden_bundle_verifies_and_rejects_tampering() {
    ...
    // Signature tamper — the exact p256 `verify`/`verify_prehash` call sites this
    // upgrade touched, exercised on real production-shaped bundle bytes rather
    // than only the synthetic rcgen fixtures above.
    let mut b = blob.clone();
    flip_b64(&mut b, "/messageSignature/signature");
    assert!(
        run_golden(&binary, &b, &prov, &trust).is_err(),
        "tampered cosign blob signature must be refused"
    );

    let mut p = prov.clone();
    flip_b64(&mut p, "/dsseEnvelope/signatures/0/sig");
    assert!(
        run_golden(&binary, &blob, &p, &trust).is_err(),
        "tampered DSSE provenance signature must be refused"
    );
...
/// The golden battery above tamper-tests the pinned CT-log key by nothing more
/// than the generic `flip_b64` (only on `certificate`/`SET`, never on the
/// trust root itself). A single byte flipped in a P-256 SPKI DER key usually
/// makes it fail to *parse*, which is refused by `trust.rs`'s "no usable
/// ctlog keys" fail-closed check — a different code path from `sct.rs`'s
/// actual `Verifier::verify` signature check. Swap in a syntactically valid
/// but different key instead, so this is refused for the SCT-verification
/// reason specifically (the lead's ask: a tampered SCT that is refused only
/// incidentally proves nothing about SCT verification).
#[test]
fn golden_bundle_sct_rejected_by_verification_not_by_an_unusable_key() {
    let binary = golden_bytes("agent-binary");
    let blob = golden_json("agent.cosign.sigstore.json");
    let prov = golden_json("agent.provenance.sigstore.json");
    let mut trust_json = golden_json("trusted_root.json");

    let wrong_key = p256::ecdsa::SigningKey::generate();
    let spki = wrong_key.verifying_key().to_public_key_der().unwrap();
    *trust_json
        .pointer_mut("/ctlogs/0/publicKey/rawBytes")
        .unwrap() = Value::String(b64(spki.as_bytes()));

    let trust = TrustRoot::from_trusted_root_json(&serde_json::to_vec(&trust_json).unwrap())
        .expect("a syntactically valid substitute key must still load");
    assert_eq!(
        trust.ctlog_keys.len(),
        1,
        "the swapped key must still parse as usable -- otherwise this test degenerates \
         into the same 'unusable key' path as the byte-flip case"
    );

    let err = run_golden(&binary, &blob, &prov, &trust).unwrap_err();
    assert!(
        matches!(err, VerifyError::Sct(_)),
        "must be refused specifically by SCT verification, not another check: {err:?}"
    );
}
