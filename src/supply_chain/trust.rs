//! Pinned Sigstore Fulcio CA certificates + Rekor keys, loaded from operator-supplied
//! `trusted_root.json`. Verification is fully offline and deterministic.
...
/// Bounds are **inclusive** of endpoints (protobuf-specs); absent `end` = open-ended.
...
/// The `valid_for` window stops a retired-then-compromised key from forging a SET for a fresh event.
#[derive(Clone)]
pub struct RekorKey {
...
/// `log_id` is SHA-256 of the key's SubjectPublicKeyInfo (RFC 6962 LogID).
#[derive(Clone)]
pub struct CtLogKey {
    pub key: p256::ecdsa::VerifyingKey,
    pub log_id: [u8; 32],
...
impl TrustRoot {
    pub fn from_trusted_root_json(bytes: &[u8]) -> Result<Self, VerifyError> {
        ...
        // exact control this hardens), so fail closed at load. An empty-DECLARED
        // ctlogs stays "SCT optional" (sigstore-go compat) but is warned + enforced
        // per-policy at verify time.
