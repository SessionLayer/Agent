//! Landlock restricts the calling thread and every thread spawned *after* it (there
//! is no TSYNC for Landlock); seccomp is applied with TSYNC. The binary therefore
//! applies hardening **before it builds the tokio runtime** (`main`), so every
//! worker inherits both the Landlock domain and the seccomp filter. Seccomp is
//! installed **last** so the Landlock syscalls run before the filter is active and
//! never need to be on the allow-list.
