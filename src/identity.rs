//! Agent mTLS X.509 identity lifecycle (Design §4, §8).
//!
//! A deliberate port + generalization of the Gateway module
...
/// Adopted Agent credential: mTLS identity + CA chain for CP verification.
#[derive(Clone)]
...
/// Locally-generated keypair + PKCS#10 CSR (key never leaves; zeroized on drop).
...
/// Generate keypair + CSR with CN and dNSName SAN. The CN is load-bearing: CP validates CN == node_name.
...
/// data-dir MUST be node-local (see the Agent runbook, `docs/operations/agent-runbook.md`
...
impl IdentityStore {
    ...
    /// Persist atomically then return credential (persist-before-adopt at §8.2).
...
/// Enroll: generate keypair+CSR, attest via JoinMethod, persist issued identity (gen 0).
#[tracing::instrument(
    name = "agent.enroll",
    skip_all,
    fields(
...
/// Renew: generate a fresh keypair+CSR, then [`renew_with_keypair`].
pub async fn renew(
    store: &IdentityStore,
    params: &ChannelParams,
    current: &Credential,
    ...
    renew_with_keypair(store, params, current, &kc).await
}

/// Renew using a caller-supplied keypair/CSR: call RenewAgentIdentity, verify
/// generation == current+1, persist. Split out of [`renew`] so the renew-ahead
/// loop can pin the SAME CSR across a Transient retry — the CP recognizes a
/// lost-response retry by its CSR public key (a clone cannot reproduce it; it
/// holds its own keypair) and replays the already-issued cert instead of
/// auto-locking. A fresh CSR per retry would make that recognition a no-op.
#[tracing::instrument(
    name = "agent.renew",
    skip_all,
    fields(
...
pub async fn renew_with_keypair(
    store: &IdentityStore,
    params: &ChannelParams,
    current: &Credential,
    kc: &KeypairCsr,
...
/// Convert Unix epoch seconds to SystemTime with checked arithmetic (fail closed on overflow).
...
/// Validate validity window: non-negative, no overflow, not_before <= not_after (fail closed).
...
/// Classify renewal errors: terminal gRPC/GenerationMismatch stops loop; else keep credential + retry (fail closed).
...
/// Post-renewal minimum-interval floor (F2): cap at remaining/2 unless zero remaining, then full floor.
...
/// Apply ±50% jitter to retry backoff (prevent thundering herd; F5).
...
impl RenewHandle {
    /// Request immediate renewal (manual trigger, FR-JOIN-4).
...
/// Renew-ahead loop configuration.
#[derive(Debug, Clone)]
...
impl RenewAhead {
    ...
    /// Run loop until shutdown, return why stopped (for exit code selection).
    pub async fn run(
        mut self,
        mut shutdown: impl std::future::Future<Output = ()> + Unpin,
    ) -> RenewOutcome {
        let mut just_renewed = false;
        // Carries a Transient failure's CSR to the next attempt so a lost-response
        // retry presents the SAME key the CP may already have issued against.
        // Keyed by generation defensively; only cleared by a successful renewal.
        let mut pending_kc: Option<(u64, KeypairCsr)> = None;
        loop {
            ...
            let delay = if just_renewed {
                ...
                if remaining.is_zero() {
                    ...
                    // corrupts the very counter clone-detection relies on, so a
            ...
            let kc = match pending_kc.take() {
                Some((generation, kc)) if generation == current.generation => kc,
                _ => match generate_keypair_and_csr(&current.node_name) {
                    Ok(kc) => kc,
                    Err(e) => {
                        tracing::warn!(error = %e, "renew-ahead: keypair/CSR generation failed, will retry");
                        tokio::select! {
                            biased;
            ...
            match renew_with_keypair(&self.store, &self.config.channel, &current, &kc).await {
                Ok(new_cred) => {
                    tracing::info!(
                ...
                Err(e) => match classify_renew_error(&e) {
                    RenewalDisposition::Transient => {
                        ...
                        // Pin this CSR to the retry: the CP may already have committed
                        // this exact request (a lost/late response), and presenting a
                        // fresh key on the retry would look identical to a clone racing
                        // the old generation.
                        pending_kc = Some((current.generation, kc));
                        tokio::select! {
                            biased;
                            _ = &mut shutdown => return RenewOutcome::Shutdown,
                            _ = tokio::time::sleep(jittered_backoff(
