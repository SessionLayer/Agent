...
/// Deliberately NOT `Debug`/`Clone`: it carries the private key.
...
/// otherwise an orchestrator sees exit 0 and silently restarts into a slow
/// crash-loop with no operator signal.
        ...
        loop {
            ...
            let delay = if just_renewed {
                ...
                if remaining.is_zero() {
                    ...
                    // fleet-wide the moment a CP clock jumps ahead.
                    tracing::error!(
                        ...
                        "RENEW-STORM GUARD: the Control Plane issued a certificate with \
                         ...
                         The generation counter is a clone-detection signal; a storm \
                         would corrupt it."
                    );
                }
            ...
            match renew_with_keypair(&self.store, &self.config.channel, &current, &kc).await {
                ...
                Err(IdentityError::GenerationMismatch { expected, got }) => {
                    tracing::error!(
                        expected,
                        got,
                        "SECURITY: generation mismatch on renewal - the identity is auto-locked by the Control Plane (possible clone); stopping renew-ahead, operator re-provision required"
                    );
                ...
                Err(e) => match classify_renew_error(&e) {
                    ...
                    _ => {
                        tracing::error!(
                            error = %e,
                            "REPAIR-NEEDED: renewal rejected by the Control Plane (locked / unknown cert / stale generation) - stopping renew-ahead; re-provision required"
                        );
