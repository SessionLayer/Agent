//! CP <-> Agent mTLS channel construction (Session Twelve).
//!
//! The control plane runs over **TLS 1.3, mutually authenticated** (VERSIONING
//! §7, Design §8/§15). A deliberate port of the Session-Four Gateway module
//! (`Gateway/gateway-core/src/mtls.rs`) — the Agent and Gateway solve the same
//! CP-facing-identity problem. Two tonic channels:
//!
//! - [`connect_bootstrap`] — a **server-authenticated** channel (no client
//!   certificate) for `AgentIdentity.EnrollAgent` before the Agent has a
//!   CP-issued identity (the join proof is the credential — the bootstrap
//!   exception). The server is verified against an operator-provided anchor.
//! - [`connect_mtls`] — a **mutually-authenticated** channel presenting the
//!   Agent's issued client certificate, for `RenewAgentIdentity`.
//!
//! Both are **fail-closed** (NFR-2): a missing/invalid/expired/wrong-CA server
//! certificate, a SAN mismatch, a plaintext peer, or a TLS-1.2 peer is refused —
//! there is never a fallback to plaintext. Every phase is time-bounded so a hung
//! peer cannot hang the Agent (§10.3).
...
/// Every variant is a refusal: no success path falls back to plaintext.
...
pub enum MtlsError {
    ...
    /// The trust anchor(s) could not be assembled into a usable verifier. A
    /// channel with no trust anchor would verify nothing, so this fails closed.
    ...
    /// — wrong-CA/expired/missing server cert, SAN mismatch, plaintext or TLS-1.2
    /// peer, or a connection error. All fail closed.
    ...
    /// The connect did not complete within its bound — a hung peer must never
    /// hang the Agent (fail closed, §10.3).
...
pub struct ChannelParams {
    ...
    pub endpoint: String,
    /// The server name (SNI + the SAN the server certificate must carry).
...
/// The Agent's mTLS client identity (issued leaf certificate + its private key),
/// PEM-encoded for tonic. The private key is held in a [`Zeroizing`] buffer so it
/// is scrubbed on drop; it never leaves the Agent.
#[derive(Clone)]
...
impl ServerCertVerifier for Tls13OnlyPinnedVerifier {
    fn verify_server_cert(
        ...
        // webpki does the chain-to-anchor path build, validity window, and SAN
        // match. Wrong-CA / expired / name-mismatch all surface as an error → the
    ...
    fn verify_tls12_signature(
        ...
        // TLS 1.3 only: this callback fires only on a TLS-1.2 handshake; refusing
        // it aborts any 1.2 negotiation. A 1.3-capable peer never reaches here.
...
/// for `EnrollAgent`. The CP server certificate is verified against
/// `trust_anchors_der` (the operator's bootstrap anchor), TLS 1.3, SAN checked.
...
/// client certificate + key) for `RenewAgentIdentity`. The CP server certificate
/// is verified against `trust_anchors_der` (the CA chain obtained at enrollment).
...
async fn connect(
    ...
    // The verifier owns all trust; do NOT also set ca_certificate (tonic returns
    // VerifierConflict). `domain_name` drives SNI and is the ServerName the
    // verifier checks the SAN against.
    ...
    // Belt-and-suspenders wall-clock bound over the whole connect so no single
    // phase can hang us even if an inner bound is missed.
...
/// for loading an operator-provided bootstrap CA file. Non-certificate PEM blocks
/// are ignored; an empty result is an error (nothing to trust).
...
/// DER-encode a single X.509 certificate as a PEM `CERTIFICATE` block, to feed
/// the enrollment-issued leaf (returned as DER) to tonic's `Identity` (PEM only).
