...
async fn agent_dials_out_and_registers_on_the_gateway_control_channel() {
    ...
    let component = hello.component.expect("HELLO carries ComponentInfo");
...
async fn agent_dials_back_and_splices_bytes_to_its_local_target() {
    ...
    // The fast-fail result arrives at splice-live, not at session end.
...
async fn a_failed_local_dial_fast_fails_so_the_gateway_need_not_wait() {
    ...
    assert_eq!(
        ...
        "the Gateway must learn the node's sshd is down immediately ('node offline')"
    );
}
...
async fn agent_holds_two_diverse_channels_and_survives_losing_one_gateway() {
    ...
    assert!(r.accepted, "gw-B must keep serving after gw-A is lost");
...
async fn a_terminal_identity_outcome_drains_live_splices_instead_of_killing_them() {
    ...
    assert_eq!(
        ...
        "the terminal exit-code contract must survive a supervisor restructure"
    );
    drop(shutdown_tx);
}
