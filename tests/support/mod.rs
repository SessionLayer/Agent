...
struct MockState {
    ...
    cert_ttl: Duration,
    next_id: u64,
    /// Agents whose NEXT renew_agent_identity call should fail transiently
    /// (Unavailable, no state mutated) — simulates a CP outage/lost response.
    fail_next_renew: HashSet<String>,
    /// (agent_id, CSR DER) for every renew_agent_identity call, in order —
    /// lets a test prove a retry reused the same CSR.
    renew_csr_log: Vec<(String, Vec<u8>)>,
}
...
impl AgentIdentity for MockSvc {
    ...
    async fn renew_agent_identity(
        ...
        st.renew_csr_log
            .push((agent_id.clone(), req.pkcs10_csr.clone()));
        if st.fail_next_renew.remove(&agent_id) {
            return Err(Status::unavailable(
                "injected transient failure (test hook)",
            ));
        }
        ...
        // Enforce it here so a renew-CSR regression is caught.
...
impl MockCp {
    ...
    pub async fn start() -> MockCp {
        ...
        let state = Arc::new(Mutex::new(MockState {
            ...
            next_id: 1,
            fail_next_renew: HashSet::new(),
            renew_csr_log: Vec::new(),
        }));
    ...
    /// already-expired (`not_after == now`) — the renew-storm condition.
    ...
    /// Fail the NEXT renew_agent_identity call for this agent with `Unavailable`
    /// and no state mutation — simulates a transient CP outage / lost response
    /// so a test can prove the caller's retry behavior.
    pub fn fail_next_renew(&self, agent_id: &str) {
        self.state
            .lock()
            .unwrap()
            .fail_next_renew
    ...
    /// The CSR DER presented on every renew_agent_identity call for this agent,
    /// in order (including calls that failed) — for asserting a retry reused
    /// the same CSR rather than a fresh keypair.
    pub fn recorded_renew_csrs(&self, agent_id: &str) -> Vec<Vec<u8>> {
        self.state
            .lock()
            .unwrap()
            .renew_csr_log
