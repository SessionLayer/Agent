...
fn glibc_hostname_resolution_survives_the_filter() {
    ...
    // skip getaddrinfo, which is exactly how the earlier gap hid). The only failure
    // mode here is a seccomp SIGSYS - a network/DNS failure returns an error, not a
    // kill, so there is no false failure where DNS is unavailable.
