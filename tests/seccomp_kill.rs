...
fn ioctl_is_arg_restricted_to_the_resolver_requests() {
    // Regression guard: the filter must allow `ioctl(fd, FIONREAD)` (glibc's UDP-DNS
    // receive path needs it) but KILL any other ioctl request — proving the fix is
    // arg-restricted, not blanket.
    let program = testing::compile_seccomp().expect("compile the seccomp program");
