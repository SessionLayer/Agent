...
# rust-toolchain.toml and both base images by digest. The digests are the
# multi-arch INDEX digests, not a platform manifest, so aarch64 still resolves;
# pinning a per-platform digest would silently break it. Tags are kept
# alongside for legibility only.
#
# Keep this tag's version equal to rust-toolchain.toml's channel. When they
# diverge the build still succeeds — rustup honours rust-toolchain.toml and
# silently fetches the pinned toolchain over the network — so the drift is
# invisible while defeating the point of shipping a pinned base image.
# ---- Builder ---------------------------------------------------------------
